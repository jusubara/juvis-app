...
export default function PayCalculatorPage() {
  return <ComingSoon title="파일럿 급여 계산기" titleEn="Pilot Pay Calculator" icon="✈️" tag="FINANCE" href="/pay-calculator" />;
}

function ComingSoon({
  title, titleEn, icon, tag,
}: {
  title: string; titleEn: string; icon: string; tag: string; href: string;
}) {
  return (
    <main className="min-h-screen grid-bg flex flex-col items-center justify-center relative">
      ...
      <div className="relative z-10 text-center px-6">
        ...
        <p className="text-[10px] font-mono text-cyan-600 tracking-widest mb-2">[{tag}]</p>
        <h1 className="text-2xl font-bold text-cyan-300 font-mono glow-text mb-1">{title}</h1>
        <p className="text-xs text-cyan-600 font-mono mb-10">{titleEn}</p>
        ...
        <p className="mt-6 text-sm text-cyan-600/60 font-mono">
          이 모듈은 개발 중입니다.
        </p>
      </div>
    </main>
